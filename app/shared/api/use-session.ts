...
//
// Returns the request-scoped client too (distinct from the `authClient` singleton in
// auth-client.ts), so callers that also sign out (see use-auth-buttons.ts) reuse the same
// client rather than creating a second one with its own auth state.
export async function useAuthSession() {
  const requestAuthClient = useAuth();

  const { data: session } = await requestAuthClient.useSession((url, opts) =>
  ...
  return { session, requestAuthClient };
}
