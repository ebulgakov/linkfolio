...
// state of a schema-only base branch (NEON_RESET_SOURCE_BRANCH_ID) via Neon's
// branch-restore API — never restored from main/prod, so the test branch
// never carries real user data. Used by e2e/global-setup.ts and
...
//
// NEON_RESET_SOURCE_BRANCH_ID must be re-synced (re-run migrations) whenever
// main's schema changes — see the schema-sync note in CLAUDE.md. Auth is a
// separate story: Neon Auth is project-scoped, not per-branch, so signups
// during the test still land in *prod*'s neon_auth.user table regardless of
// this reset — see e2e/auth.spec.ts.
