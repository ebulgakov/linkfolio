...
- `app/widgets/app-header/`, `app/widgets/app-footer/` — app-wide layout widgets, used only from `app.vue`. Each composes an independent `features/` slice plus local, static presentational sub-components that live directly in the widget's own `ui/` (no logic, not extracted as a slice): `app-header` composes `AuthButtons` (feature) + local `ui/app-logo.vue`; `app-footer` composes `LocaleSwitcher` (feature) + local `ui/footer-copyright.vue`/`ui/footer-links.vue`. Each widget exports its single public component via `index.ts` (`export { default as AppHeader } from "./ui/app-header.vue"`) — import via that barrel, never deep paths. Per FSD v2.1 `widgets` is discouraged by default (see `.claude/skills/feature-sliced-design/references/layer-structure.md`), but justified here because both compose multiple independent flows rather than being a single-purpose block — that's the litmus test for reaching for `widgets` vs. keeping something as plain app-layer UI
