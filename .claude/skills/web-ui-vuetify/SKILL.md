---
name: web-ui-vuetify
description: Idiomatic patterns, conventions, and anti-patterns for building Vue 3 UIs with Vuetify (Material Design components) -- theming, the defaults/v-defaults-provider system, SASS customization, data tables, form validation, layout, and composables like useTheme/useDisplay/useRules. Use whenever writing, editing, or reviewing Vue code that uses Vuetify components (v-btn, v-card, v-data-table, v-text-field, etc.) or createVuetify(), even if the user doesn't say "Vuetify" explicitly. For a component's exact/current props, events, and slots, pair this with the "vuetify" skill instead of guessing.
metadata:
...
> **Quick Guide:** Vuetify provides 80+ pre-styled Vue 3 components implementing Material Design. Configure with `createVuetify()` -- set `theme` for colors, `defaults` for global component props, and `blueprint: md3` for MD3 compliance. Use `v-defaults-provider` for scoped prop overrides. Customize at the SASS level with `@use 'vuetify/settings'` for compile-time changes. **Targets the Vuetify 3.x line** -- `useRules` composable for form validation, date picker improvements, performance optimizations. Vuetify releases frequently (3.x is past 3.13 as of mid-2026, and a 4.x line also exists) and composable behavior has changed across even patch versions (e.g. `useRules`), so treat specific version numbers and edge-case composable behavior here as a starting point, not ground truth -- check `package.json` for the installed version, and for exact, always-current component props/events/slots use the **vuetify** skill's lookup scripts instead of relying on memory. Vuetify is **template-driven** -- prefer declarative props/slots over imperative JS.
...
**(For a component's exact/current props, types, and defaults -- especially anything not covered in the patterns below -- use the `vuetify` skill's `scripts/vuetify-component.sh` rather than relying on memory. This skill covers idiomatic patterns; that one covers the precise, version-matched API surface.)**
...
- **`vuetify` skill** (separate skill, if installed) -- exact, version-matched props/events/slots for any component via `scripts/vuetify-component.sh`, plus directives and install steps. Use it alongside this one: this skill teaches _how_ to structure Vuetify code well, that one confirms the _exact API_ for whatever component you're touching.
...
**(For exact/current prop names, types, and defaults, confirm against the `vuetify` skill rather than this file's memory-written examples.)**
